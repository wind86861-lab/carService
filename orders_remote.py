...
async def close_order_endpoint(
    ...
    from bot.database.models import get_master_total_earnings as _get_total, get_user_by_id
    master_user = await get_user_by_id(master["id"])
    custom_pct = master_user.get("master_share_percent") if master_user else None
    master_total = await _get_total(master["id"])
    financials = calculate_order_financials(agreed, body.parts_cost, master_total, custom_master_pct=custom_pct)
    await close_order(
        order_number=order_number,
...
async def add_expense(
    ...
    from web.utils.photos import validate_image, save_upload_file
    order = await get_order_by_number(order_number)
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    if order["status"] == "closed":
    ...
    if not item_name.strip():
        raise HTTPException(status_code=400, detail="Nomi majburiy")
    if amount <= 0:
    ...
    await create_order_expense(
        ...
        amount=int(amount),
        receipt_file_id=receipt_filename,
        added_by=master["id"],
    )
    await update_parts_cost(order_number, int(amount), changed_by=master["id"])
    updated = await get_order_by_number(order_number)
    expenses = await get_expenses_by_order(updated["id"])
    return {
        "parts_cost": float(updated["parts_cost"]),
        "expenses": [
            {
                "id": e["id"], "item_name": e["item_name"], "amount": float(e["amount"]),
                "receipt_url": get_photo_url(e["receipt_file_id"]) if e.get("receipt_file_id") else None,
