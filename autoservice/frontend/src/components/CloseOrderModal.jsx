...
export default function CloseOrderModal({ order, onClose, onConfirm, loading }) {
  ...
  const handleConfirm = () => {
    if (!partsCost || isNaN(parts) || parts < 0) {
      setError('Enter a valid parts cost (can be 0)')
      return
    }
    setError('')
    onConfirm(parts)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h2 className="text-lg font-bold">Close Order {order?.order_number}</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-green-700">Master share (40%)</span>
              <span className="font-semibold text-green-700">{fmt(masterShare)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-blue-700">Service share (60%)</span>
          ...
          {profit < 0 && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">
              ...
              <span>Negative profit: parts cost exceeds agreed price. You can still close the order.</span>
            </div>
          )}
        </div>
