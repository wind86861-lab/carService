...
import { adminCreateOrder, getMastersList, adminUploadPhotos } from '../../api/admin'
import { getCarHistory } from '../../api/client'
...
export default function AdminNewOrder() {
  ...
  const [form, setForm] = useState({
    master_id: '', brand: '', model: '', plate: '', color: '', year: '',
  ...
  useEffect(() => {
    getMastersList().then(setMasters).catch(console.error)
  }, [])
  ...
  const set = (field) => (e) => {
    ...
    if (field === 'plate' && val.length >= 4) {
      ...
      debounceRef.current = setTimeout(() => {
        getCarHistory(val).then(setHistory).catch(() => setHistory([]))
      }, 500)
    } else if (field === 'plate') {
      setHistory([])
    }
  ...
  const validate = () => {
    const e = {}
    if (!form.master_id) e.master_id = 'Ustani tanlang'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const body = {
        ...form,
        master_id: parseInt(form.master_id),
        year: parseInt(form.year),
        agreed_price: parseFloat(stripSpaces(form.agreed_price)),
        paid_amount: parseFloat(stripSpaces(form.paid_amount)),
      }
      const result = await adminCreateOrder(body)
      if (photos.length > 0) {
        await adminUploadPhotos(result.order_number, photos)
      }
        ...
        <div className="p-6 flex items-center justify-center min-h-[60vh]">
          <div className="card max-w-md w-full text-center">
            ...
            <div className="mt-4 flex gap-2">
              <button onClick={() => navigate(`/admin/orders/${success}`)} className="btn-secondary flex-1">Buyurtmani ko'rish</button>
              <button onClick={() => navigate('/admin')} className="btn-secondary flex-1">Boshqaruv</button>
            </div>
          </div>
        </div>
      </AdminLayout>
      ...
      <div className="p-3 sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl space-y-4 sm:space-y-6">
          ...
          <div className="card space-y-4">
            <h2 className="font-semibold text-gray-700">Usta tanlash</h2>
            <Field label="Usta *" error={errors.master_id}>
              <select className="input" value={form.master_id} onChange={set('master_id')}>
                <option value="">Ustani tanlang...</option>
                {masters.map(m => (
                  <option key={m.id} value={m.id}>
                    {m.full_name || m.username || `ID: ${m.id}`}{m.phone ? ` (${m.phone})` : ''}
                  </option>
                ))}
              </select>
            </Field>
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => navigate('/admin/orders')} className="btn-secondary flex-1">Bekor qilish</button>
