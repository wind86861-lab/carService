...
import { getAdminOrderDetail, forceCloseOrder, adminRecordPayment, adminAddExpense, adminDeleteOrder } from '../../api/admin'
...
export default function AdminOrderDetail() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(e.response?.data?.detail || 'O\'chirib bo\'lmadi')
  ...
  return (
    <AdminLayout>
      ...
      <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-5xl">
                ...
                <tbody>
                  {expenses.map(e => (
                    <tr key={e.id} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="py-2 pr-3 text-gray-500 text-xs">{e.added_by_name} ({e.added_by_role === 'admin' ? 'Admin' : 'Usta'})</td>
                      <td className="py-2">
                        {e.receipt_url
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await adminAddExpense(orderNumber, expenseName.trim(), amt, expenseFile)
                          setExpenseName('')
                          setExpenseAmount('')
                          setExpenseFile(null)
                          await reload()
        ...
        {expenses.length === 0 && order.status !== 'closed' && (
          <div className="card">
            <h2 className="font-semibold text-gray-700 mb-3">🔩 Ehtiyot qismlar va Xarajatlar</h2>
            <p className="text-sm text-gray-500 mb-4">Hozircha xarajatlar yo'q.</p>
            <div className="space-y-3">
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  type="text"
        ...
        {order.status !== 'closed' && (
          <div className="card border-2 border-red-100">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <button onClick={() => setShowDeleteConfirm(true)} className="btn-danger bg-red-600 ml-auto sm:ml-0">🗑️ Buyurtmani o'chirish</button>
            </div>
          </div>
        )}
      ...
      {showDeleteConfirm && (
        <ConfirmDialog
          ...
          message={`${orderNumber} buyurtma va unga bog'liq barcha ma'lumotlar (rasmlar, loglar, xarajatlar) butunlay o'chiriladi. Bu amalni ASLO qaytarib bo'lmaydi.`}
